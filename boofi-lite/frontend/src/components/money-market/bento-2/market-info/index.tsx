...
const MarketInfo: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const fetchedAssetData = await fetchAssetData(currentViewTab as 'lend' | 'withdraw' | 'borrow' | 'repay');
              ...
              <TableRow key={index} className="font-nupower">
                ...
                <TableCell>
                  <div className="flex -space-x-1">
                    {[1, 2, 3].map((_, i) => (
                      <Skeleton key={i} className="w-4 h-4 rounded-full border-2 border-background" />
                    ))}
                  </div>
                </TableCell>
                <TableCell className="text-xs"><Skeleton className="h-3 w-16" /></TableCell>
                <TableCell className="text-xs"><Skeleton className="h-3 w-12" /></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
