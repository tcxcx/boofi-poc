...
contract WormholeSimulator {
     ...
     */
    function fetchSignedMessageFromLogs(Vm.Log memory log, uint32 emitterChainId)
        public
        view
    ...
    function setMessageFee(uint256 newFee) public {
        ...
        bytes memory message = abi.encodePacked(coreModule, uint8(3), uint32(wormhole.chainId()), newFee);
