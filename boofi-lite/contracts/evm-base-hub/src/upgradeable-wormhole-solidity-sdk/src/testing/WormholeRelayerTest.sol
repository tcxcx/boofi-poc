...
abstract contract WormholeRelayerTest is Test {
    ...
    mapping(uint32 => ChainInfo) public chainInfosTestnet;
    mapping(uint32 => ChainInfo) public chainInfosMainnet;

    // active forks for the test
    mapping(uint32 => ActiveFork) public activeForks;
    uint32[] public activeForksList;
    ...
    function _setActiveForks(ChainInfo[] memory chainInfos) internal virtual {
        ...
        activeForksList = new uint32[](chainInfos.length);
    ...
    function _setUp() internal {
        ...
        for (uint256 i = 0; i < activeForksList.length; ++i) {
            uint32 chainId = activeForksList[i];
    ...
    function createAndAttestToken(uint32 homeChain) public returns (ERC20Mock token) {
    ...
    function mintUSDC(uint32 chain, address addr, uint256 amount) public {
        uint256 originalFork = vm.activeFork();
...
abstract contract WormholeRelayerBasicTest is WormholeRelayerTest {
    ...
    function setTestnetForkChains(uint32 _sourceChain, uint32 _targetChain) public {
        ChainInfo[] memory forks = new ChainInfo[](2);
    ...
    function setMainnetForkChains(uint32 _sourceChain, uint32 _targetChain) public {
        ChainInfo[] memory forks = new ChainInfo[](2);
    ...
    function setForkChains(bool testnet, uint32 _sourceChain, uint32 _targetChain) public {
			if (testnet) {
