...
import {IBooFiPriceOracle, IBooFiPriceSource, AggregatorV3Interface} from "../../interfaces/IBooFiPriceOracle.sol";

/**
 * @title BooFiPriceOracle
 */
contract BooFiPriceOracle is IBooFiPriceOracle, BaseBooFiPriceSource {
    ...
    event PriceSourceSet(address indexed asset, IBooFiPriceSource priceSource, uint256 maxPriceAge);
    ...
    ) BaseBooFiPriceSource(_outputAsset) {
    ...
    function priceAvailable(address _asset) public view override returns (bool) {
        return sources[_asset].priceSource != IBooFiPriceSource(address(0));
    }

    function getPrice(address _asset) public view override returns (IBooFiPriceOracle.Price memory price) {
        return getPrice(_asset, sources[_asset].maxPriceAge);
    }

    function getPrice(address _asset, uint256 _maxAge) public view override returns (IBooFiPriceOracle.Price memory price) {
    ...
    function setPriceSource(
        ...
        if (_priceSource.priceSource == IBooFiPriceSource(address(0))) {
            revert InvalidPriceSource();
        }
    ...
    function removePriceSource(address _asset) external onlyOwner {
        if (sources[_asset].priceSource == IBooFiPriceSource(address(0))) {
            revert InvalidPriceSource();
        }
        delete sources[_asset];
        emit PriceSourceSet(_asset, IBooFiPriceSource(address(0)), 0);
    }
