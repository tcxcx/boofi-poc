...
interface IWormholeRelayerBase {
    ...
    function getRegisteredWormholeRelayerContract(uint32 chainId) external view returns (bytes32);
...
error DeliveryProviderDoesNotSupportTargetChain(address relayer, uint32 chainId);
...
error InvalidEmitter(bytes32 emitter, bytes32 registered, uint32 chainId);
...
error TargetChainIsNotThisChain(uint32 targetChain);
