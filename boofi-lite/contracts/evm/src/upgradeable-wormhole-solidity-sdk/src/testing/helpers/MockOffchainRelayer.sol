...
contract MockOffchainRelayer {
    ...
    uint32 chainIdOfWormholeAndGuardianUtilities;
    ...
    mapping(uint32 => address) wormholeRelayerContracts;

    mapping(uint32 => uint256) forks;

    mapping(uint256 => uint32) chainIdFromFork;
    ...
    function relay(
        ...
        uint32 chainId = chainIdFromFork[vm.activeFork()];
        require(
        ...
        for (uint32 i = 0; i < encodedSignedVaas.length; i++) {
    ...
    function genericRelay(
        ...
        if (payloadId == 1) {
            ...
            uint32 targetChain = instruction.targetChain;
            ...
            uint32 targetChain = decodeDeliveryInstruction(
