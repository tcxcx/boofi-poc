...
import "./BaseBooFiPriceSource.sol";

/**
 * @title ChainedPriceSource
 * @notice An adapter to chain multiple IBooFiPriceSource price feeds into a single price feed
 */
contract ChainedPriceSource is BaseBooFiPriceSource {
    struct ChainedSource {
        IBooFiPriceSource source;
        address inputAsset;
        string outputAsset;
        uint256 maxPriceAge;
    }
    ...
    ) BaseBooFiPriceSource(_sources[_sources.length - 1].outputAsset) {
